...
double tsp_prog_dyn(point *V, int n, int *Q) {
    /*
    Version programmation dynamique du TS 32 bits) ne seront pas assez grands
          pour représenter tous les sous-ensembles;
        - pour n=32, il faudra P. Le résultat (la tournée
  ...
  */
    ...
    do {
        for(t = 0; t< n-1; t++){//calcul de D[t][S]
            int S_minus_t = DeleteSet(S,t);
            if (S == S_minus_t) continue; //{//si t est contenu dans S
            D[t][S].length = DBL_MAX;
            for(int i = 0; i< n-1; i++){//pour chaque i contenu dans S\{t}
                if (S_minus_t != DeleteSet(S_minus_t,i)){
                    double l = D[i][S_minus_t].length + dist(V[t],V[i]);
                    if (D[t][S].length > l){
                        D[t][S].length = l;
                        D[t][S].pred = i;
                    }
    ...
    if (running) {
        ...
        w = -1;
        for (t = 0; t < n-1; t++ ){
            double l = D[t][K].length + dist(V[n-1],V[t]);
            if(w == -1 || l < w){
                final_t = t;
                w = l;
            }
        }
        ExtractPath(D, final_t, K, n, Q);
    }
    else{
        w = -1;
    }
